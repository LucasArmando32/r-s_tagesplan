export const LOCALES = ["de", "es"];
export const DEFAULT_LOCALE = "de";

export const dictionaries = {
  de: {
    appName: "RS Tagesplan",
    common: {
      save: "Speichern",
      cancel: "Abbrechen",
      edit: "Bearbeiten",
      delete: "Löschen",
      archive: "Archivieren",
      create: "Neu anlegen",
      loading: "Lädt…",
      yes: "Ja",
      no: "Nein",
      warehouse: "Lager",
      free: "Frei",
      logout: "Abmelden",
      name: "Name",
      address: "Adresse",
      travelTimeMinutes: "Reisezeit ab Lager (Min.)",
      notes: "Besondere Hinweise",
      status: "Status",
      active: "Aktiv",
      inactive: "Inaktiv",
      save_error: "Etwas ist schiefgelaufen. Bitte nochmal versuchen.",
      confirm_delete: "Wirklich endgültig löschen?",
      none: "—",
    },
    login: {
      title: "Anmeldung",
      subtitle: "Nur für die Geschäftsleitung",
      email: "E-Mail",
      password: "Passwort",
      submit: "Anmelden",
      error: "E-Mail oder Passwort ist falsch.",
    },
    board: {
      title: "Tagesplan",
      subtitle: "Arbeiter per Drag & Drop den Baustellen zuordnen",
      no_workers: "Keine Arbeiter zugeteilt",
      new_site: "Baustelle",
      new_worker: "Arbeiter",
      new_car: "Auto",
      notes_placeholder: "z. B. Zugang über den Hinterhof, Kunde ist den ganzen Tag vor Ort…",
      loading_screen: "Ladebildschirm für Arbeiter",
      loading_screen_active: "Aktiv – Arbeiter sehen den Ladebildschirm",
      loading_screen_inactive: "Inaktiv – Arbeiter sehen den Tagesplan",
      reason_frei: "Frei",
      reason_ferien: "Ferien",
      reason_krank: "Krank",
    },
    containers: {
      title: "Mulden",
      new: "Neue Mulde",
      edit: "Mulde bearbeiten",
      empty: "Noch keine Mulden angelegt.",
      location: "Standort",
      full: "Voll",
      not_full: "Nicht voll",
      mark_full: "Als voll melden",
      mark_not_full: "Als nicht voll melden",
    },
    tasks: {
      title: "Aufgaben",
      new: "Neue Aufgabe",
      edit: "Aufgabe bearbeiten",
      empty: "Noch keine Aufgaben angelegt.",
      description: "Beschreibung",
      assigned_to: "Zugeteilt an",
      unassigned: "Niemandem zugeteilt",
      date: "Datum",
      done: "Erledigt",
      not_done: "Offen",
      mark_done: "Als erledigt markieren",
      mark_not_done: "Als offen markieren",
    },
    public: {
      title: "Tagesplan",
      subtitle: "Übersicht für heute",
      sites_heading: "Baustellen",
      containers_heading: "Mulden",
      tasks_heading: "Aufgaben",
      no_sites: "Aktuell keine aktiven Baustellen.",
      no_containers: "Aktuell keine Mulden erfasst.",
      no_tasks: "Aktuell keine Aufgaben für heute.",
      address: "Adresse",
      notes: "Hinweise",
      not_ready_title: "Tagesplan wird vorbereitet",
      not_ready_message:
        "Der Tagesplan ist jeden Tag spätestens um 12:00 Uhr bereit.",
    },
    history: {
      title: "Verlauf",
      subtitle: "Wo jeder Arbeiter an einem bestimmten Tag war",
      empty: "Für diesen Tag gibt es noch keine Einträge.",
      worker: "Arbeiter",
      location: "Ort",
      type_lager: "Lager",
      type_frei: "Frei",
      type_ferien: "Ferien",
      type_krank: "Krank",
      tab_day: "Tag",
      tab_month: "Monat",
      month_subtitle: "Anzahl Tage pro Arbeiter in diesem Monat",
      month_empty: "Für diesen Monat gibt es noch keine Einträge.",
      col_ferien: "Ferien",
      col_krank: "Krank",
      col_frei: "Frei",
      col_lager: "Lager",
      col_obra: "Baustelle",
      export_csv: "Als CSV exportieren",
    },
  },
  es: {
    appName: "RS Tablero de Obras",
    common: {
      save: "Guardar",
      cancel: "Cancelar",
      edit: "Editar",
      delete: "Borrar",
      archive: "Archivar",
      create: "Crear nuevo",
      loading: "Cargando…",
      yes: "Sí",
      no: "No",
      warehouse: "Almacén",
      free: "Libre",
      logout: "Cerrar sesión",
      name: "Nombre",
      address: "Dirección",
      travelTimeMinutes: "Tiempo de viaje desde el almacén (min.)",
      notes: "Notas especiales",
      status: "Estado",
      active: "Activo",
      inactive: "Inactivo",
      save_error: "Algo salió mal. Intentá de nuevo.",
      confirm_delete: "¿Borrar definitivamente?",
      none: "—",
    },
    login: {
      title: "Iniciar sesión",
      subtitle: "Solo para la jefa",
      email: "Correo electrónico",
      password: "Contraseña",
      submit: "Entrar",
      error: "Correo o contraseña incorrectos.",
    },
    board: {
      title: "Tablero de obras",
      subtitle: "Arrastrá a los obreros para asignarlos a una obra",
      no_workers: "Sin obreros asignados",
      new_site: "Obra",
      new_worker: "Obrero",
      new_car: "Auto",
      notes_placeholder: "ej. cuidado, acceso por el patio trasero, cliente presente todo el día…",
      loading_screen: "Pantalla de carga para obreros",
      loading_screen_active: "Activa – los obreros ven la pantalla de carga",
      loading_screen_inactive: "Inactiva – los obreros ven el tablero",
      reason_frei: "Libre",
      reason_ferien: "Vacaciones",
      reason_krank: "Enfermo",
    },
    containers: {
      title: "Contenedores",
      new: "Nuevo contenedor",
      edit: "Editar contenedor",
      empty: "Todavía no hay contenedores cargados.",
      location: "Ubicación",
      full: "Lleno",
      not_full: "No lleno",
      mark_full: "Marcar como lleno",
      mark_not_full: "Marcar como no lleno",
    },
    tasks: {
      title: "Tareas",
      new: "Nueva tarea",
      edit: "Editar tarea",
      empty: "Todavía no hay tareas cargadas.",
      description: "Descripción",
      assigned_to: "Asignada a",
      unassigned: "Sin asignar",
      date: "Fecha",
      done: "Hecha",
      not_done: "No hecha",
      mark_done: "Marcar como hecha",
      mark_not_done: "Marcar como no hecha",
    },
    public: {
      title: "Tablero de obras",
      subtitle: "Resumen de hoy",
      sites_heading: "Obras",
      containers_heading: "Contenedores",
      tasks_heading: "Tareas",
      no_sites: "Por ahora no hay obras activas.",
      no_containers: "Por ahora no hay contenedores cargados.",
      no_tasks: "Por ahora no hay tareas para hoy.",
      address: "Dirección",
      notes: "Notas",
      not_ready_title: "Preparando el tablero de obras",
      not_ready_message:
        "El tablero está listo todos los días a más tardar a las 12:00.",
    },
    history: {
      title: "Historial",
      subtitle: "Dónde estuvo cada obrero un día determinado",
      empty: "Todavía no hay registros para este día.",
      worker: "Obrero",
      location: "Ubicación",
      type_lager: "Almacén",
      type_frei: "Libre",
      type_ferien: "Vacaciones",
      type_krank: "Enfermo",
      tab_day: "Día",
      tab_month: "Mes",
      month_subtitle: "Cantidad de días por obrero en este mes",
      month_empty: "Todavía no hay registros para este mes.",
      col_ferien: "Vacaciones",
      col_krank: "Enfermo",
      col_frei: "Libre",
      col_lager: "Almacén",
      col_obra: "Obra",
      export_csv: "Exportar como CSV",
    },
  },
};

export function getDictionary(locale) {
  return dictionaries[locale] || dictionaries[DEFAULT_LOCALE];
}
