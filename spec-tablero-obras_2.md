...
> Nota: esta es una aplicación **independiente** de la app de registro de horas (ver documento aparte `spec-horas-trabajadores.md`) — cada una es su propio proyecto Next.js con su propia base de datos SQLite (ver sección 2).
...
- **Next.js** (JavaScript, no TypeScript) — frontend y backend integrados (API routes / Server Actions). Contenedor Docker propio para esta app.
- **SQLite** — base de datos embebida (un solo archivo en disco, sin proceso/contenedor propio), leída/escrita directamente por el backend de Next.js. No hace falta Postgres ni Supabase — el volumen de datos de esta app (obras, obreros, contenedores, tareas) es pequeño y con muy poca escritura concurrente.
- **Autenticación propia** (no Supabase Auth): solo existe la cuenta de la jefa — login con usuario/contraseña con hash, sesión con cookie. Los dos toggles públicos (contenedor lleno/no lleno, tarea hecha/no hecha) son rutas de servidor abiertas, sin necesidad de sesión.
- **Tailwind CSS** — estilos.
- **Idiomas:** alemán y español, misma pauta que la app de horas.
- **Despliegue:** Dokploy (Docker), aplicación separada de la de horas (su propio contenedor Next.js), con su propio subdominio para la página pública.

> Nota sobre por qué SQLite y no Supabase: el servidor actual (Hetzner, 3.7GB RAM) ya está bastante justo de recursos (swap al 75% de uso) y aloja varios proyectos más. Un stack completo de Supabase (10+ contenedores) fácilmente suma 1-2GB de RAM — arriesgado ahí. Esta app en particular tiene muy poco volumen de datos y solo una cuenta con login, así que SQLite (cero contenedores extra, cero RAM constante) es más que suficiente y evita el riesgo de sobrecargar el servidor.
...
- rol (`admin`)
- activo
- password_hash (contraseña guardada con hash, ej. bcrypt)
...
- Sin Row Level Security de base de datos (SQLite no lo tiene) — estas reglas se aplican **en el código del backend**: solo la `jefa` (con sesión válida) puede crear, editar o borrar obras, obreros, contenedores y tareas.
- **Excepción controlada:** la página pública permite dos acciones muy específicas sin login — cambiar `lleno` en un contenedor y `hecha` en una tarea. Esto se resuelve con dos rutas de servidor (API routes de Next.js) dedicadas únicamente a esas dos acciones puntuales, sin sesión requerida — así nadie sin login puede tocar nada más, aunque intente manipular la página.
- La página pública no expone el archivo SQLite ni ninguna credencial al navegador — el servidor de Next.js hace las consultas internamente y solo entrega el HTML/datos ya filtrados.
...
- App Next.js en su propio contenedor Docker vía Dokploy, separada de la app de horas.
...
> Nota sobre backups: a diferencia de la app de horas, esta app **no** necesita un backup automático adicional — sus datos (obras, obreros, contenedores, tareas del día) no son críticos ni difíciles de reconstruir si se perdieran. El volumen persistente (protege contra reinicios/redeploys) es suficiente aquí.
